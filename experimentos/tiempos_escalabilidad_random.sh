...
for d in $DIRS; do
    ...
    for suf1 in $SUFFIXES; do
        for suf2 in $SUFFIXES; do
            ${CODE}/k2tree_setop_intersection_estrategiaparalela ${DATA}/${PREFIXES}${suf1}${d} ${DATA}/${PREFIXES}${suf2}${d} 3 borrar >> ${FILE}
        done
        echo " " >> ${FILE}
    done
    echo " " >> ${FILE}
...
for d in $DIRS; do
    ...
    for suf1 in $SUFFIXES; do
        for suf2 in $SUFFIXES; do
            ${CODE}/k2tree_setop_intersection_parallel ${DATA}/${PREFIXES}${suf1}${d} ${DATA}/${PREFIXES}${suf2}${d} 3 borrar >> ${FILE}
        done
        echo " " >> ${FILE}
    done
    echo " " >> ${FILE}
